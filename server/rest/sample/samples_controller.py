...
class BioSampleApi(Resource):
    ...
    def post(self, accession):
        response = samples_service.create_biosample_from_accession(accession)
        return Response(json.dumps(response['message']), mimetype="application/json", status=response['status'])

    @jwt_required()
    def delete(self,accession):
        deleted_accession = samples_service.delete_biosample(accession)
...
class BioSamplesApi(Resource):
    ...
    def get(self):
        total, data = samples_service.get_biosamples(**request.args)
