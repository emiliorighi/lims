<template>
    <div>
        <ModelForm v-if="showModelForm" :mode="editMode" :incoming-model="model" :existing-models="models"
            :project-id="projectId" @submit="handleEditSubmit" @close="showModelForm = false" />
        <div v-else>
            <div class="row align-center">
                ...
                <div v-if="role === 'admin' || role === 'project_manager'" class="flex">
                    <div class="row">
                        <div class="flex">
                            <VaDropdown placement="bottom-start" v-model="showEditDropdown" stick-to-edges
                                :disabled="isArchived">
                                <template #anchor>
                                    ...
                                    <VaCardContent class="pa-0">
                                        <div class="pa-2" style="width: 300px;">
                                            <template v-if="totalRecords > 0">
                                                <div class="pa-2 cursor-pointer hover-bg"
                                                    @click="handleEditWithMode('edit-controlled')">
                                                    <h6 class="va-h6 mb-1">Controlled Mode</h6>
                                                    <p class="va-text-secondary mb-0">Safe mode that prevents breaking
                                                        changes to existing records and dependencies.</p>
                                                </div>
                                                <VaDivider />
                                                <div class="pa-2 cursor-pointer hover-bg"
                                                    @click="handleEditWithMode('edit-override')">
                                                    <h6 class="va-h6 mb-1">Override Mode</h6>
                                                    <p class="va-text-secondary mb-0">Advanced mode that allows all
                                                        changes but may break existing records and dependencies.</p>
                                                </div>
                                            </template>
                                            <template v-else>
                                                <div class="pa-2 cursor-pointer hover-bg"
                                                    ...
                                                    <p class="va-text-secondary mb-0">Make changes to your model
                                                        structure.</p>
                                                </div>
                                            </template>
                                            <VaDivider />
                                            <div class="pa-2 cursor-pointer hover-bg" @click="handleClone">
                                                <h6 class="va-h6 mb-1">Clone Model</h6>
                                                <p class="va-text-secondary mb-0">Create a copy of this model with all
                                                    its attributes and settings.</p>
                                            </div>
                                        </div>
                                    </VaCardContent>
                                </VaCard>
            ...
            <VaTabs color="textPrimary" class="va-text-capitalize" v-model="viewValue" grow>
                <template #tabs>
...
const model = computed(() => modelStore.currentModel || undefined)
...
async function handleEditSubmit(updatedModel: ResearchModel) {
    try {
        if (editMode.value.startsWith('edit-')) {
