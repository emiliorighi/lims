<template>
    <VaModal max-height="500px" fixed-layout :model-value="!!attributeStore.attribute" @ok="submitAttribute"
        ...
        <VaForm v-if="attributeStore.attribute" ref="attributeForm">
            <div>
                <VaInput class="mb-4" label="filter key (required)" :rules="[(v: string) => attributeStore.attributes.findIndex(it => it.key === v) === -1 || attributeStore.attributes.findIndex(it => it.key === v) === attributeStore.attributeId || 'Key must be unique',
    (v: string) => v.length > 0 || 'Key is required!']" placeholder="Type a value.."
                    v-model="attributeStore.attribute.key" />
                <VaInput class="mb-4" placeholder="Type a value.." :rules="[(v: string) => attributeStore.attributes.findIndex(it => it.label === v) === -1 || attributeStore.attributes.findIndex(it => it.label === v) === attributeStore.attributeId || 'label must be unique',
    (v: string) => v.length > 0 || 'Label is required!']" v-model="attributeStore.attribute.label"
                    label="Attribute label (required)" />
                <VaInput class="mb-4" v-model="attributeStore.attribute.description"
                    label="filter description (optional)" placeholder="Type a brief description of the project" />
                <VaSelect class="mb-4" label="Is required?" v-model="attributeStore.attribute.required"
                    :options="[true, false]" />
                <div class="row row-equal">
                    <div class="flex lg12 md12">
                        <VaCard square outlined>
                            <VaCardTitle>Type</VaCardTitle>
                            <VaCardTitle>
                                <VaSelect style="width: 100px;" v-model="fType" :options="fieldTypes" />
                            </VaCardTitle>
