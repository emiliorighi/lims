...
const formInfo = computed(() => idToUpdate.value ? {
    ...
    title: 'Create Record',
    description: `Create a new ${props.model.name}`
})
...
async function handleSubmit() {
    ...
    try {
        loading.value = true
        if (idToUpdate.value) {
            const { data } = await AuthService.updateRecord(props.projectId, props.model.name, idToUpdate.value, Object.fromEntries(filteredEntries))
            init({ message: 'Record correctly updated', color: 'success' })
        } else {
            const { data } = await AuthService.createRecord(props.projectId, props.model.name, Object.fromEntries(filteredEntries))
