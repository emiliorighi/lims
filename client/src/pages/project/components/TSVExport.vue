    ...
    <VaModal closeable hide-default-actions fixed-layout v-model="recordStore.showReportModal">
        <template #header>
            <div class="row align-center justify-space-between">
                ...
                <VaIcon color="primary" size="large" class="flex" name="fa-file-export" />
            </div>
        </template>
...
const options = computed(() => schemaStore.schema?.models.find(m => props.modelName === m.name)?.fields.map(({ key }) => key) ?? [])
